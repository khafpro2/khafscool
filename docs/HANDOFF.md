...
10. **Module 3 pilote** — `acmt-exam-prep` MP4 FR ; `app-protection-conditional-access` YouTube `F4PESZiEQhU` (whitelist `fr`).
11. **Module 4 pilote** — `apps-vpp-management` et `vpp-abm-business-apps` YouTube `k0cchC6mE88` (ABM/apps) ; `api-automation-advanced-policies` YouTube `t3j9TkFfUJw` (pas de titre ADE sur module 1 Apple).
