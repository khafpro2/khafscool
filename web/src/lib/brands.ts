...
/** Icône Jamf seule (sans wordmark) — pour tuiles carrées avec libellé à côté. */
export const JAMF_SYMBOL_PATHS: BrandPath[] = [
  {
...
export const JAMF_SYMBOL_VIEWBOX = '0 0 48 22.2';

export const JAMF_WORDMARK_PATHS: BrandPath[] = [
  {
...
export const BRAND_PATHS: Record<BrandId, BrandPath[]> = {
  ...
  jamf: [...JAMF_SYMBOL_PATHS, ...JAMF_WORDMARK_PATHS],
  microsoft: [
...
/** `symbol` = icône seule (défaut Jamf en tuile) ; `full` = logo complet avec wordmark. */
export type BrandMark = 'symbol' | 'full';
...
const JAMF_SYMBOL_ASPECT = 48 / 22.2;

export function resolveBrandMark(brand: BrandId, mark: BrandMark = 'symbol'): BrandMark {
  if (brand !== 'jamf') return 'full';
  return mark;
}

export function getBrandPaths(brand: BrandId, mark: BrandMark = 'symbol'): BrandPath[] {
  if (brand === 'jamf') {
    return mark === 'full' ? BRAND_PATHS.jamf : JAMF_SYMBOL_PATHS;
  }
  return BRAND_PATHS[brand];
}

export function getBrandViewBox(brand: BrandId, mark: BrandMark = 'symbol'): string {
  if (brand === 'jamf') {
    return mark === 'full' ? BRAND_VIEWBOXES.jamf : JAMF_SYMBOL_VIEWBOX;
  }
...
export function getBrandIconDimensions(
  brand: BrandId,
  size: BrandIconSize,
  mark: BrandMark = 'symbol',
): { width: number; height: number } {
  const px = BRAND_SIZE_PX[size];
  const resolvedMark = resolveBrandMark(brand, mark);

  if (brand === 'jamf' && resolvedMark === 'symbol') {
    return {
      width: Math.round(px * JAMF_SYMBOL_ASPECT),
      height: px,
    };
  }
